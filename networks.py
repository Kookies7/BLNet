...
def define_D(input_nc, ndf, which_model_netD,
             n_layers_D=5, norm='instance', use_sigmoid=False, gpu_ids=[], patch=False):
    netD = None
    use_gpu = len(gpu_ids) > 0
...
class SmoothLoss(nn.Module):
    ...
    def r_smooth(self,input_R):
        input_R = 0.299 * input_R[:, 0, :, :] + 0.587 * input_R[:, 1, :, :] + 0.114 * input_R[:, 2, :, :]
        input_R = torch.unsqueeze(input_R, dim=1)
        return torch.mean(torch.abs(self.gradient(input_R, "x"))  +
                          torch.abs(self.gradient(input_R, "y") ))
...
class Blur(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.conv2d(x, self.weight.cuda(), stride=1, padding=10, groups=self.nc)
        return x
...
class ColorLoss(nn.Module):
    ...
    def color(self,pred_reflect, true_reflect):
        blur_rgb = Blur(3)
        pred_reflect=blur_rgb(pred_reflect)
        true_reflect=blur_rgb(true_reflect)
        return torch.sum(torch.pow((true_reflect - pred_reflect), 2)).div(2 * true_reflect.size()[0])
