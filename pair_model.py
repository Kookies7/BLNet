...
class PairModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.smooth_loss=networks.SmoothLoss(opt)
        self.color_loss=networks.ColorLoss(opt)
    ...
    def forward(self):
        ...
        #self.real_A_gray = torch.cat([self.real_A,self.real_A_gray], 1)
    ...
    def backward(self, epoch):
        ...
        self.loss_MSE = torch.mean(torch.abs(self.fake_B-self.real_B))
        self.loss_vgg1 = self.vgg_loss.compute_vgg_loss(self.vgg, self.fake_B, self.real_B)
        self.loss_vgg2 = self.vgg_loss.compute_vgg_loss(self.vgg, self.fake_B, self.input)
        self.loss_vgg = self.loss_vgg1 / self.loss_vgg2
        #self.smooth_loss = F.smooth_l1_loss(self.gray, self.real_A_gray_o)
        self.loss_somooth=self.smooth_loss.smooth(self.gray, self.fake_B)
        self.loss_color=self.color_loss.color(self.fake_B, self.real_B)
        #loss=torch.nn.CrossEntropyLoss()
        #self.loss_texture=loss(self.fake_B,torch.squeeze(self.real_B).long())


        self.loss = self.loss_MSE + self.loss_vgg + self.loss_somooth#+0.01*self.loss_color
        self.loss.backward()
